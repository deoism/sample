...
mongoClient.open(function(err, mongoClient) {
    db = mongoClient.db("navigationdb09");
    db.collection('navigation', {strict:true}, function(err, collection) {
        if (err) {
            console.log("The 'navigation' collection doesn't exist. Creating it with sample data...");
            populateDB();
        }
    });
});
...
exports.findById = function(req, res) {
    ...
    db.collection('navigation', function(err, collection) {
        collection.findOne({'index': navigation.length}, function(err, item) {
            console.log(item);
            res.jsonp(item);
        });
    });
...
exports.findByManager = function(req, res) {
    ...
    db.collection('navigation', function(err, collection) {
        collection.find({'myList': ul}).toArray(function(err, items) {
            console.log(items);
            res.jsonp(items);
        });
    });
...
exports.findAll = function(req, res) {
    ...
    db.collection('navigation', function(err, collection) {
        if (name) {
            collection.find({"#mylist li a:label": new RegExp(name, "<li>")}).toArray(function(err, items) {
                res.jsonp(items);
            });
        } else {
...
var populateDB = function() {
    ...
    db.collection('navigation', function(err, collection) {
        collection.insert(navigation, {safe:true}, function(err, result) {});
    });
